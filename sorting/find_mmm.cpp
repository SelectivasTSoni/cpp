...
//finds median value in an unsorted array arr
...
void minMaxMedianSort(int min, int max, int median, vector<int> arr, vector<int>& finalarr)
{
	...
	for (int i = 0; i < 7; i++) {
		if (arr[i] >= min && arr[i] < median) // don't include median
		{   
...
int main()
{
	...
	minMaxMedianSort(min, max, median, arr, finalarr);
